...
Embedding methods need training triples. The program uses two data sources.

```text
The three profession RDF graphs are the project input checked by the program.
The BIM Spatial Models dataset provides the training examples.
```

The model is trained on element-type patterns, not on exact object IDs.
...
### 2.1 Positive And Negative Samples

The training script reads positive examples from the BIM Spatial Models dataset.

Positive examples are relationship rows from `spatial_relationships_detailed.csv`.
...
Negative samples are generated by replacing the object type inside the same dataset project.
...
The first row is a positive example. The second row is a generated negative example. Negative means the model should give that type pattern a lower score.

### 2.2 Features From The RDF Graph
...
### 2.3 How The Dataset Is Used

The BIM Spatial Models dataset is used because it already contains spatial relationship labels.
