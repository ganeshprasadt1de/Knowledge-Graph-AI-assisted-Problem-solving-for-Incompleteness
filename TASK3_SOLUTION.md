...
A separate ComplEx profession-graph example is included in:

```text
scripts/complex_profession_demo.py
```

This script creates pseudo-labels from the three RDF graphs. A pseudo-label is an automatically generated training label. In this project, pseudo-labels come from geometry/type rules, for example:

```text
(MEP duct element, penetrates, structural beam)
(MEP duct element, requiresOpeningIn, structural beam)
(architectural wall, sameAs, structural wall)
```

It trains a small ComplEx-style embedding model on those pseudo-labels and writes:

```text
outputs/05_complex_profession_predictions.json
```

This is useful for demonstrating how ComplEx produces plausibility scores from the actual profession graphs. It is still not a scientifically reliable trained model, because the labels are generated by rules rather than manually verified ground truth.
...
Demonstrating the input and output format
```

The current graph is not sufficient for:

```text
Training a reliable embedding model
Comparing TransE, RotatE, and ComplEx scientifically
